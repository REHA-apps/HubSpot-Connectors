...
async def hubspot_events(request: Request):
    ...
    from app.integrations.slack.actions import (
        SlackConnector,  # Local import to avoid circular dep if any
    )
    ...
    for event in events:
        await connector.send_event(
            {"type": event.get("eventType"), "object_id": event.get("objectId")}
        )
