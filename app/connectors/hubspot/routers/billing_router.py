...
async def hubspot_trial(
    ...
    log.info("Processing 7-day trial signup for portal_id=%s", portal_id)
    ...
    log.info("Trial workspace initialized. Redirecting to HubSpot OAuth.")
...
async def create_checkout_session(
    ...
    log = CorrelationAdapter(logger, corr_id)
    log.info("Creating Stripe Checkout session for portal_id=%s", portal_id)
    ...
    try:
        ...
        log.error("Failed to create Stripe session: %s", e)
...
async def checkout_success(
    ...
    """
    log = CorrelationAdapter(logger, corr_id)
    log.info("Checkout success for session=%s", session_id)
...
async def stripe_webhook(
    ...
    if event.type == "checkout.session.completed":
        ...
        if session["mode"] == "subscription":
            ...
            log.info("Activated PRO for workspace=%s", workspace_id)
    # Handle the event
