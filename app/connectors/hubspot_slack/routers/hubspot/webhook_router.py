...
async def handle_app_uninstall(
    ...
    if not integration:
        logger.warning("No integration found for portal_id=%s on uninstall", portal_id)
        return {"status": "not_found"}

    workspace_id = integration.workspace_id

    # Downgrade to FREE and mark as canceled
    ...
    logger.info(
        "Workspace %s downgraded to FREE after app uninstall (portal_id=%s)",
        workspace_id,
        portal_id,
    )
