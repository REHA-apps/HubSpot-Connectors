...
class InteractionService:
    ...
    async def handle_interaction(self, payload: Mapping[str, Any]) -> Any:
        ...
        try:
            ...
            if not team:
                logger.error("Missing team in Slack interaction payload")
                return
            ...
            if not integration:
                logger.error("No integration found for Slack team_id=%s", team_id)
                return

            # Initialize MessagingService with this specific integration
            ...
            logger.error("Failed to handle Slack interaction: %s", exc, exc_info=True)
    ...
    async def _handle_block_actions(
        ...
        logger.info("Processing block_action action_id=%s value=%s", action_id, value)

        # Handle Gated Feature Clicks
        if action_id.startswith("gated_feature_click:"):
            feature_id = action_id.split(":", 1)[1]
        ...
        if action_id in (
            ...
            return await self._handle_ticket_action(
                action_id, payload, integration, messaging_service
            )
        ...
        logger.warning("Unhandled action_id=%s", action_id)
