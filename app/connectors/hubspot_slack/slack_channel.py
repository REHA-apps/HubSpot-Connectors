...
class SlackChannel(BaseChannel):
    ...
    async def resolve_channel_name(self, name: str) -> str | None:
        ...
        try:
            ...
            logger.error("Failed to resolve Slack channel name '%s': %s", name, exc)
    ...
    async def get_default_channel_id(self) -> str | None:
        ...
        try:
            ...
            while True:
                ...
                for channel in channels:
                    if channel.get("is_general"):
                        channel_id = str(channel.get("id"))
                        logger.info("Resolved default channel to %s", channel_id)
                        return channel_id
            ...
            logger.error("Failed to fetch default channel: %s", exc)
    ...
    async def apps_uninstall(self) -> bool:
        ...
        try:
            ...
            logger.error("Slack apps.uninstall exception: %s", exc)
            return False
    ...
    async def get_user_by_email(self, email: str) -> str | None:
        ...
        try:
            ...
            logger.error("Failed to lookup Slack user by email '%s': %s", email, exc)
        return None
    ...
    async def send_via_response_url(
        ...
        try:
            ...
            logger.error("Failed to send to Slack response_url: %s", exc)
            return False
