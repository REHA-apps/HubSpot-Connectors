...
async def slack_interactions(
    ...
    if interaction_type == "block_actions":
        response = await _handle_block_actions(
            payload, log, integration_service, corr_id
        )
        if response:
            return response

    if interaction_type in ("shortcut", "message_action"):
        response = await _handle_shortcuts(payload, log, integration_service, corr_id)
        if response:
            return response
...
async def _handle_block_actions(
    ...
    if not is_pro:
        ...
        if response_url:
            try:
                ...
                log.error("Failed to send upgrade prompt: %s", exc)
        return Response(status_code=200)
    ...
    log.info("Fast-path: opening modal for trigger=%s", trigger_id[:8])
    try:
...
async def _handle_shortcuts(
    ...
    if not is_pro:
        try:
            ...
            log.error("Failed to open upgrade modal: %s", exc)
        return Response(status_code=200)
    ...
    log.info("Fast-path: opening creation modal for trigger=%s", trigger_id)
    try:
        client = await integration_service.get_slack_client(integration)
        await client.views_open(trigger_id=trigger_id, view=modal)
    except Exception as exc:
