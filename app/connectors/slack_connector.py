...
class SlackConnector(Connector):
    ...
    async def normalize_event(
        ...
        return NormalizedEvent(
            ...
            identity=Identity(
                id=str(raw_event.get("user") or raw_event.get("actor_id") or "unknown"),
                source="slack",
            ),
            payload=dict(raw_event),
            timestamp=str(raw_event.get("event_ts") or raw_event.get("ts")),
        )
    ...
    async def send_message(
        ...
        """Send a Slack message using the Slack Web API."""
        ...
        # Use the correct Slack API method
        return await self.client.chat_postMessage(**payload)
